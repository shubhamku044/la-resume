...
interface HeadingProps {
  data: TechProResumeData['heading'];
...
const HeadingSection = ({ data, setTempData, setIsChangesSaved }: HeadingProps) => {
  ...
  const handleChange = (field: keyof TechProResumeData['heading'], value: string) => {
    setTempValues((prev) => ({
      ...prev,
      [field]: value,
    }));
  ...
  const handleAddSocial = () => {
    ...
    setTempValues((prev) => ({
      ...prev,
      socials: [...prev.socials, { label: newSocialName, url: newSocialUrl }],
    }));
    setNewSocialName('');
    setNewSocialUrl('');
    if (setIsChangesSaved) setIsChangesSaved(false);
