...
interface ResumeFormProps {
  ...
  templateSampleData: TechProResumeData;
  templateFunction: (data: TechProResumeData) => string;
  slug: string;
  title: string;
...
const ResumeForm = ({
  ...
  const { resumeSampleData } = useResumeData('techpro');
  const [resumeSectionsOrder, setResumeSectionsOrder] = useState(
    formData?.sectionOrder || (resumeSampleData as TechProResumeData).sectionOrder
  );
  const { user } = useUser();
  ...
  const sections = Object.keys(formData) as Array<keyof TechProResumeData>;
